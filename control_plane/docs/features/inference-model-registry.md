...
Registry này thuộc Model Quality và không còn xuất hiện trong Annotation Canvas.
Dropdown `Inference model của tôi` cùng prompt inference runtime đã bị loại bỏ.
Table `UserInferencePreference` chỉ còn được giữ để không phá dữ liệu Quality cũ
và sẽ được xử lý trong một migration deprecation riêng.

Automatic Annotation dùng registry độc lập tại `/system/auto-annotation/`, trong
đó model/function khai báo CVAT-compatible label spec và được map sang Project
schema ở cấp Task. Upload model Quality không tự biến model thành Auto Annotation
function.
...
Model sau đó dùng cho workflow Model Quality. Muốn dùng trong Annotation, AI Admin
phải deploy inference endpoint và đăng ký function/spec ở Auto Annotation Registry.
